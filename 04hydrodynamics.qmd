...
# Given
internal_diameter_mm = 75  # mm
velocity_m_per_s = 1.2     # m/s
relative_density = 0.9
density_water_kg_per_m3 = 1000  # standard value

# Convert diameter to metres
internal_diameter_m = internal_diameter_mm / 1000
radius_m = internal_diameter_m / 2
...
# Mass flow rate in tonnes per hour (1 tonne = 1000 kg, 1 hour = 3600 s)
...
# The difference in water levels, or equivalent head, 
# creates a hydrostatic pressure in the connecting legs 
# that counteracts the mercury column. 
# Subtracting 1 from the mercury column height 
# compensates for this pressure difference.
h_eq = h_mercury * (SG - 1)
...
::: {#exm-fe2-3}
Water is pumped to a height of 20 m at a rate of 12 litres per second. Determine the power required. If the pump efficiency is 75%, calculate the input power.
...
theoretical_power = m_dot * g * height           # watts
input_power = theoretical_power / efficiency     # watts

# Results
print(f"Theoretical (hydraulic) power required: {theoretical_power:.4f} W ({theoretical_power / 1000:.4f} kW)")
...
# Theoretical (hydraulic) power delivered to the water (W)
theoretical_power = input_power * efficiency

# Head h (m) using P = m_dot * g * h
h = theoretical_power / (m_dot * g)

# Results
print(f"Volumetric flow rate (v_dot): {v_dot:.4f} m³/s")
print(f"Mass flow rate (m_dot): {m_dot:.1f} kg/s")
